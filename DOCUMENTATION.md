...
SEO Pro can generate a physical `public/robots.txt` file. Head to `Tools > SEO Pro > Crawling & AI` to configure general crawler rules, AI crawler access, Content Signals, and sitemap discovery, then select **Generate**. SEO Pro validates the policy, writes the file, and only then persists the settings.

Managed mode provides presets and separate controls for AI search crawlers, user-directed agents, and model-training crawlers. Custom source mode writes the text you enter without generating any additional directives. SEO Pro normalizes line endings and ensures that the file ends with a newline. The preview does not write the file or persist settings.

Content Signals express preferences for search indexing, AI input or grounding, and model training. These directives are voluntary signals and are not a technical access control. Protect private content with authentication or other server-side authorization.

The policy is shared by every Statamic site using the same public directory. In multisite installations, SEO Pro adds one deduplicated `Sitemap:` line for each site authority.

If an existing `public/robots.txt` file was not generated by SEO Pro, the Control Panel displays a warning and offers to import it into custom mode. Generating always asks for confirmation and overwrites the physical file.

The generated file should be committed to the Statamic project's repository. When Statamic's automatic Git integration is enabled, SEO Pro includes `public/robots.txt` in the tracked paths and dispatches a `RobotsTxtGenerated` event. Otherwise, commit the generated file normally.

You may also regenerate the file from the command line, which is useful during builds or deployments:

```shell
php please seo-pro:generate:robots-txt
```

The target public directory must be writable when generation runs. Robots settings and generation metadata are stored in `resources/addons/seo-pro.yaml` with the other addon settings.
