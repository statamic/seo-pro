...
// Check if Google can find a favicon for the domain. If it can't, lets use a fallback.
const fetchFavicon = async () => {
	const url = `https://www.google.com/s2/favicons?domain=${domain.value}&sz=64}`;
				...
				<div class="min-w-0">
					<a class="flex flex-row items-center mb-1.5" :href="url" target="_blank">
						<div class="size-[28px] bg-[#f3f5f6] !border !border-[#d2d2d2] dark:!border-[#5c5f5e] rounded-[50%] mr-3">
							<img v-if="faviconSrc" class="size-full" :src="faviconSrc" />
							<div v-if="faviconErrored" class="p-1">
